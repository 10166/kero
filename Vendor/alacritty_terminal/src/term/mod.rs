...
impl<T> Term<T> {
    /// Read-only checkpoint access. Never swap the live grids while taking a
    /// checkpoint: entering the alternate screen would erase its contents.
    pub fn inactive_grid(&self) -> &Grid<Cell> { &self.inactive_grid }
    pub fn checkpoint_scroll_region(&self) -> Range<Line> { self.scroll_region.clone() }
    pub fn checkpoint_tabs(&self) -> &[bool] { &self.tabs.tabs }
    pub fn checkpoint_charset(&self) -> CharsetIndex { self.active_charset }
    pub fn checkpoint_keyboard_stacks(&self) -> (&[KeyboardModes], &[KeyboardModes]) {
        (&self.keyboard_mode_stack, &self.inactive_keyboard_mode_stack)
    }
    pub fn checkpoint_titles(&self) -> (&Option<String>, &[Option<String>]) {
        (&self.title, &self.title_stack)
    }
