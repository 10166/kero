...
impl SshConnection {
    ...
    /// Kero group collapse closes the transport, leaving remote daemon PTYs alive.
    pub async fn disconnect(&self) {
        self.mark_dead();
        let _ = self.handle.lock().await.disconnect(
            russh::Disconnect::ByApplication, "Kero group detached", "en",
        ).await;
    }
