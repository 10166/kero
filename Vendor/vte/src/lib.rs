...
pub struct Parser<const OSC_RAW_BUF_SIZE: usize = MAX_OSC_RAW> {
    ...
    /// Incomplete escape only; completed output is represented by the grid.
    #[cfg(feature = "std")]
    checkpoint_pending: Vec<u8>,
    #[cfg(feature = "std")]
    checkpoint_overflow: bool,
}
...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    /// Bytes required to resume an incomplete escape or UTF-8 codepoint.
    #[cfg(feature = "std")]
    pub fn checkpoint_pending(&self) -> Option<&[u8]> {
        if self.checkpoint_overflow { return None; }
        if self.partial_utf8_len > 0 {
            Some(&self.partial_utf8[..self.partial_utf8_len])
        } else {
            Some(&self.checkpoint_pending)
        }
    }

    #[cfg(feature = "std")]
    fn checkpoint_byte(&mut self, byte: u8) {
        if byte == 0x1b {
            self.checkpoint_pending.clear();
            self.checkpoint_overflow = false;
        }
        // C0 controls execute immediately inside CSI/ESC. Their effects are
        // already in the grid and must not execute for a second time on replay.
        if byte >= 0x20 || byte == 0x1b {
            if self.checkpoint_pending.len() < 1024 * 1024 {
                self.checkpoint_pending.push(byte);
            } else { self.checkpoint_overflow = true; }
        }
    }
            ...
            match self.state {
                State::Ground => {
                    ...
                    if self.state == State::Escape {
                        self.checkpoint_pending.clear();
                        self.checkpoint_pending.push(0x1b);
                        self.checkpoint_overflow = false;
                    }
                },
                _ => {
    ...
    fn action_osc_put(&mut self, byte: u8) {
        ...
        if self.osc_raw.len() < 1024 * 1024 { self.osc_raw.push(byte); }
    }
