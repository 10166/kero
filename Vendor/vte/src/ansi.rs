...
impl<T: Timeout> Processor<T> {
    ...
    #[cfg(feature = "std")]
    pub fn checkpoint_preceding(&self) -> Option<char> { self.state.preceding_char }

    /// Parser tail at a checkpoint. Synchronized bytes are still unparsed,
    /// so the receiver must see BSU followed by exactly that buffered suffix.
    #[cfg(feature = "std")]
    pub fn checkpoint_pending(&self) -> Option<Vec<u8>> {
        let mut pending = self.parser.checkpoint_pending()?.to_vec();
        if self.state.sync_state.timeout.pending_timeout() {
            pending.extend_from_slice(b"\x1b[?2026h");
            pending.extend_from_slice(&self.state.sync_state.buffer);
        }
        Some(pending)
    }
    ...
    T: Timeout,
{
    ...
    fn csi_dispatch(
        ...
        match (action, intermediates) {
            // Kero checkpoint metadata restores REP state without painting a
            // fake character or changing a saved cursor/grid cell.
            ('z', [b'>']) if params.len() == 3 && params.iter().next().is_some_and(|p|p == [997]) => {
                let mut values=params.iter(); values.next();
                let high=values.next().and_then(|v|v.first()).copied().unwrap_or(0) as u32;
                let low=values.next().and_then(|v|v.first()).copied().unwrap_or(0) as u32;
                let value=(high<<16)|low;
                self.state.preceding_char=if value==0 {None} else {char::from_u32(value)};
            },
