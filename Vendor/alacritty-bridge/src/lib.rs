...
use kitty_graphics_tracking::{advance_cursor, advance_text, KittyGraphicsCursorTracker};
...
/// Raw bytes read from the PTY before OSC interception or terminal parsing.
/// Hosts use this for byte-transparent remote transport.
pub const KERO_EVENT_RAW_OUTPUT: u32 = 14;
/// Raw terminal input generated by a parser-only remote surface.
...
pub struct KeroConfig {
    ...
    pub cursor_shape: u8,
    pub cursor_blinking: bool,
    /// Parse output for snapshots but let a host-managed peer generate all
    /// terminal protocol replies. Used as Kero's backend-neutral PTY owner.
    pub suppress_protocol_writes: bool,
}
...
struct Proxy {
    ...
    raw_output_enabled: Arc<AtomicBool>,
}
...
impl Proxy {
    ...
    fn write_pty(&self, text: String) {
        self.write_pty_bytes(text.into_bytes());
    }

    fn write_pty_bytes(&self, bytes: Vec<u8>) {
        let shared = self.shared.lock();
        if shared.remote_transport {
            drop(shared);
            self.emit(KERO_EVENT_REMOTE_INPUT, &bytes);
            return;
        }
        drop(shared);
        ...
        if let Some(sender) = self.sender.get() {
            let _ = sender.send(GraphicsMsg::Input(bytes.into()));
        }
    }
...
impl Read for OscReader {
    fn read(&mut self, output: &mut [u8]) -> io::Result<usize> {
        ...
        if self.proxy.raw_output_enabled.load(Ordering::Acquire) {
            self.proxy.emit(KERO_EVENT_RAW_OUTPUT, &output[..count]);
        }
        if self.raw_only {
            return Ok(count);
        }
...
impl OscPty {
    fn new(inner: tty::Pty, proxy: Proxy, raw_only: bool) -> io::Result<Self> {
...
pub struct KeroTerminal {
    ...
    event_proxy: Proxy,
    term_config: Config,
    notifier: Option<GraphicsNotifier>,
    shared: Arc<FairMutex<Shared>>,
    protocol_writes_enabled: Arc<AtomicBool>,
    raw_output_enabled: Arc<AtomicBool>,
...
impl RemoteParser {
    fn advance(
        &mut self,
        terminal: &mut Term<Proxy>,
        graphics: &Arc<FairMutex<KittyGraphicsStore>>,
        graphics_size: &Arc<FairMutex<KittyGraphicsSize>>,
        event_proxy: &Proxy,
        bytes: &[u8],
    ) {
        for item in self.graphics_interceptor.process(bytes) {
            match item {
                KittyGraphicsItem::Text(text) => {
                    let track_scrolls = graphics.lock().state.has_placements();
                    let effects = advance_text(
                        &mut self.graphics_cursor_tracker,
                        &mut self.parser,
                        terminal,
                        &text,
                        track_scrolls,
                ...
                KittyGraphicsItem::Command(command) => {
                    ...
                    if let Some(response) = result.response {
                        event_proxy.write_pty_bytes(response);
                    }
...
pub unsafe extern "C" fn kero_alacritty_new(
    ...
    let protocol_writes_enabled = Arc::new(AtomicBool::new(!config.suppress_protocol_writes));
    let raw_output_enabled = Arc::new(AtomicBool::new(config.suppress_protocol_writes));
    let proxy = Proxy {
        callback,
        context: SwiftContext(context),
        sender: Arc::new(OnceLock::new()),
        shared: shared.clone(),
        protocol_writes_enabled: protocol_writes_enabled.clone(),
        raw_output_enabled: raw_output_enabled.clone(),
    };
    ...
    let pty = match OscPty::new(pty, proxy.clone(), config.suppress_protocol_writes) {
        Ok(pty) => pty,
        Err(_) => return std::ptr::null_mut(),
    };
    let event_loop = match GraphicsEventLoop::new(
        ...
        protocol_writes_enabled.clone(),
        !config.suppress_protocol_writes,
    ) {
    ...
    Box::into_raw(Box::new(KeroTerminal {
        ...
        event_proxy: proxy,
        term_config,
        notifier: Some(GraphicsNotifier(sender)),
        shared,
...
/// Creates an emulator-only terminal. Swift feeds remote PTY output through
/// `kero_alacritty_feed`; generated input and resize requests are emitted as
/// callbacks instead of touching a local process.
#[no_mangle]
pub unsafe extern "C" fn kero_alacritty_new_remote(
    config: *const KeroConfig,
    theme: *const KeroTheme,
    callback: KeroEventCallback,
    context: *mut c_void,
    ...
    let protocol_writes_enabled = Arc::new(AtomicBool::new(false));
    let raw_output_enabled = Arc::new(AtomicBool::new(false));
    let proxy = Proxy {
        callback,
        context: SwiftContext(context),
        sender: Arc::new(OnceLock::new()),
    ...
    let term_config = Config {
        scrolling_history: config.scrollback_lines.max(1),
        default_cursor_style: configured_cursor_style(config.cursor_shape, config.cursor_blinking),
        // A remote renderer must never expose the controller's clipboard to a
    ...
    Box::into_raw(Box::new(KeroTerminal {
        ...
        last_kitty_damage_revision: 0,
        exited: false,
        remote_parser: Some(RemoteParser::default()),
    }))
}

/// Feeds byte-transparent output into a parser-only remote terminal.
#[no_mangle]
pub unsafe extern "C" fn kero_alacritty_feed(
    ...
    let Some(parser) = terminal.remote_parser.as_mut() else {
        return;
    };
    parser.advance(
        &mut *terminal.term.lock(),
        &terminal.kitty_graphics,
        &terminal.kitty_graphics_size,
        &terminal.event_proxy,
        data,
    );
...
#[no_mangle]
pub unsafe extern "C" fn kero_alacritty_set_protocol_writes(
    handle: *mut KeroTerminal,
    enabled: bool,
) {
    if handle.is_null() {
...
pub unsafe extern "C" fn kero_alacritty_free(handle: *mut KeroTerminal) {
    ...
    if let Some(notifier) = terminal.notifier {
        let _ = notifier.0.send(GraphicsMsg::Shutdown);
    }
}
...
pub unsafe extern "C" fn kero_alacritty_write(
    ...
    if terminal.remote_parser.is_some() {
        terminal.event_proxy.emit(KERO_EVENT_REMOTE_INPUT, &payload);
    } else if let Some(notifier) = &terminal.notifier {
...
pub unsafe extern "C" fn kero_alacritty_resolve_clipboard(
    ...
    if !terminal.protocol_writes_enabled.load(Ordering::Acquire) {
        return;
    }
    if let Some(notifier) = &terminal.notifier {
        notifier.notify(format(text).into_bytes());
    }
}
...
pub unsafe extern "C" fn kero_alacritty_resize(
    ...
    *terminal.kitty_graphics_size.lock() = KittyGraphicsSize {
    ...
    if let Some(notifier) = &mut terminal.notifier {
        ...
        payload.extend_from_slice(&columns.max(1).to_le_bytes());
        payload.extend_from_slice(&rows.max(1).to_le_bytes());
        payload.extend_from_slice(&cell_width.max(1).to_le_bytes());
        payload.extend_from_slice(&cell_height.max(1).to_le_bytes());
        terminal
            .event_proxy
            .emit(KERO_EVENT_REMOTE_RESIZE, &payload);
    }
}
...
mod tests {
    ...
    extern "C" fn capture_event(context: *mut c_void, kind: u32, data: *const u8, len: usize) {
        let events = unsafe { &*(context as *const Mutex<Vec<(u32, Vec<u8>)>>) };
        let payload = if data.is_null() || len == 0 {
            Vec::new()
        } else {
            unsafe { std::slice::from_raw_parts(data, len) }.to_vec()
        };
        events.lock().unwrap().push((kind, payload));
    }
    ...
    #[test]
    fn remote_parser_forwards_replies_and_tracks_kitty_graphics() {
        let events: Mutex<Vec<(u32, Vec<u8>)>> = Mutex::new(Vec::new());
        let window_size = WindowSize {
            num_lines: 3,
            num_cols: 40,
            cell_width: 8,
        ...
        let proxy = Proxy {
            ...
            context: SwiftContext(&events as *const _ as *mut c_void),
            sender: Arc::new(OnceLock::new()),
            shared,
            protocol_writes_enabled,
            raw_output_enabled,
        ...
        let mut term = Term::new(Config::default(), &size, proxy.clone());
        ...
        let mut parser = RemoteParser::default();

        parser.advance(&mut term, &graphics, &graphics_size, &proxy, b"\x1b[6n");
        assert!(events
            .lock()
            .unwrap()
            .iter()
            .any(|(kind, payload)| *kind == KERO_EVENT_REMOTE_INPUT && !payload.is_empty()));
